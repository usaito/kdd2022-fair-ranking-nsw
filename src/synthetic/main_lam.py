...
def main(cfg: DictConfig) -> None:
    ...
    lambda_list = cfg.setting.lambda_list
    v = exam_func(n_doc, cfg.setting.default_K, cfg.setting.exam_func)
    pol_list = ["max", "expo-fair"]
    ...
    for lam_ in lambda_list:
        ...
        for s in np.arange(cfg.setting.n_seeds):
            ...
            for alpha_ in alpha_list:
                pi_nsw = compute_pi_nsw(rel_mat=rel_mat_obs, alpha=alpha_, v=v)
            ...
            user_util_list += [user_util_unif]
            mean_max_envy_list = [
                max_envies_max.mean(),
                max_envies_expo.mean(),
            ]
            mean_max_envy_list += mean_max_envy_nsw_list
            mean_max_envy_list += [-10]  # always zero, but to remove from the figure
            ...
            pct_item_util_better_off_list += [-10]
            ...
            pct_item_util_worse_off_list += [-10]

            result_df_ = DataFrame(
                data={
                    "policy": pol_list,
                    "seed": [s] * len(pol_list),
                    "lam": [lam_] * len(pol_list),
        ...
        logger.info(f"lam={lam_}: {elapsed}min (diff {diff}min)")
        elapsed_prev = elapsed

        result_df_box.reset_index(inplace=True)
        result_df_box.to_csv(log_path / f"result_df_box_lam={lam_}.csv")

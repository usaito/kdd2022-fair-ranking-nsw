...
def main(cfg: DictConfig) -> None:
    ...
    noise_levels = cfg.setting.noise_levels
    ...
    for noise_ in noise_levels:
        ...
        logger.info(f"noise_level={noise_}: {elapsed}min (diff {diff}min)")
        elapsed_prev = elapsed

        result_df_box.reset_index(inplace=True)
        result_df_box.to_csv(log_path / f"result_df_box_noise={noise_}.csv")
